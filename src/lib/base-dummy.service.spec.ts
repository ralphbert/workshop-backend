...
class DummyService extends BaseDummyService<Entity, CreateEntity, Partial<Entity>> {
    constructor() {
        super();
    }
...
describe('CrudStorage', () => {
    ...
    describe('add', () => {
        it('should add elements', async () => {
            ...
            expect(service.findAll().find(item => item.id === 1001)).toEqual({ id: 1001, name: 'Test' });
            ...
            expect(service.findAll().find(item => item.id === 1002)).toEqual({ id: 1002, name: 'Foo' });
